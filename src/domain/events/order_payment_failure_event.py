...
T = TypeVar("T", bound="OrderPaymentFailureEventDto")

# Define the payload schema for the event by creating a Pydantic model that describes the payment failure details.
class OrderPaymentFailurePayload(BaseModel):
...
# Use the generic BaseEventDto[OrderPaymentFailurePayload] to create the event DTO.
class OrderPaymentFailureEventDto(BaseEventDto[OrderPaymentFailurePayload]):
    """
    Represents the full event DTO for an order payment failure,
    following the structure:

        {
            ...baseEvent fields,
            payload: OrderPaymentFailurePayload
        }

    This leverages the BaseEventDto generic for strong type checking and validation.
    """
    @classmethod
    def from_payload(cls: Type[T], event: Dict[str, Any]) -> T:
        ...
        try:
            payload_obj = OrderPaymentFailurePayload(
