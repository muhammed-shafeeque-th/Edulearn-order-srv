...
class SagaOrchestrator:
    def __init__(self, steps: list[SagaStep],  logging_service: ILoggingService, metrics: IMetricsService) -> None:
        ...
        self._logger = logging_service.get_logger("SagaOrchestrator")
    ...
    async def execute(self):
        for step in self.steps:
            try:
                ...
                self._metrics.saga_failures(step=step.__class__.__name__)
                await self.compensate()
                raise
            self._logger.info("SAGA completed successfully")
