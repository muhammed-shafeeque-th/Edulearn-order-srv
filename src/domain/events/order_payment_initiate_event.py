...
T = TypeVar("T", bound="OrderPaymentInitiatedEventDto")

class OrderPaymentInitiatedPayload(BaseModel):
...
class OrderPaymentInitiatedEventDto(BaseEventDto[OrderPaymentInitiatedPayload]):
    @classmethod
    def from_payload(cls: Type[T], event: Dict[str, Any]) -> T:
        ...
        try:
            payload_obj = OrderPaymentInitiatedPayload(
